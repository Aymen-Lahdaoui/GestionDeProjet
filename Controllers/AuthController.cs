...
public class AuthController : ControllerBase
{
    ...
    public async System.Threading.Tasks.Task<IActionResult> UpdateProfile([FromBody] ProfileUpdateModel model)
    {
        ...
        // Activity Logging
        await LogActivity("Mise à jour du Profil", $"L'utilisateur a mis à jour ses informations de profil.", "User", user.Id);
    ...
    public async System.Threading.Tasks.Task<IActionResult> UpdateUserRole([FromBody] UpdateUserRoleModel model)
    {
        ...
        foreach (var user in users)
        {
            ...
            // Activity Logging
            await LogActivity("Changement de Rôle", $"Le rôle de l'utilisateur {model.Email} a été changé en '{model.NewRole}'.", "User", user.Id);
        }
    ...
    private async System.Threading.Tasks.Task LogActivity(string action, string details, string entityType = null, string entityId = null)
    {
        try
        {
            ...
            var log = new ActivityLog
            {
                ...
                // EntityId (int) is not easily used for GUID string IDs of users, so we skip it or handle differently
            };
